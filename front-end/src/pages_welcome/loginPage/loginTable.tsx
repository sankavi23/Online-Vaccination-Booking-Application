  ...
  const loginUserAccount = () => {
    ...
    login(formValues)
      .then((res: any) => {
        ...
        if (res.user?.roleName === "HOSPITAL") {
          navigate("/myAccountHP");
        } else if (res.user?.roleName === "USER") {
          navigate("/myAccount");
        } else if (res.user?.roleName === "ADMIN") {
          navigate("/userAD");
        }
      })
      .catch((err) => {
    ...
    >
      ...
      <form onSubmit={handleSubmit}>
        ...
        >
          ...
          >
            <b style={{ color: "#4b4453" }}>
              Don't you have account?
              <b style={{ color: "#2c73d2" }}> Register</b>
            </b>
          </IconButton>
        </div>
      </form>
